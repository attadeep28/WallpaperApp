...
public class Adapter extends RecyclerView.Adapter<Adapter.ViewHolder> {
    ...
    Context context;
    ArrayList<ImageModel> walpaperList;

    public Adapter(Context context, ArrayList<ImageModel> walpaperList) {
    ...
    public void onBindViewHolder(@NonNull Adapter.ViewHolder holder, @SuppressLint("RecyclerView") int position) {
        ...
        Glide.with(context).load(walpaperList.get(position).getSrc().getPortrait()).into(holder.imageView);
        holder.imageView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(context,SetWalpaper.class);
                intent.putExtra("image",walpaperList.get(position).getSrc().getPortrait());
